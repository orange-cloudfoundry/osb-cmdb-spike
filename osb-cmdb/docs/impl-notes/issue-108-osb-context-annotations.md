...
#### Formatting param as flattened keys with raw mapping without key name standardization: 
...
####  Formatting param as flattened keys with org/space/instance precedence and key name standardization:
...
#### Introducing v2 format in new `x_osb_cmdb_v2` key (while preserving existing `x-osb-cmdb` format)
